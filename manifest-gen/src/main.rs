...
fn generate_operator(args: &Args) -> Result<()> {
    ...
    let operator_role = Role {
        metadata: ObjectMeta {
            name: Some(format!("{}-role", operator_service_account_name)),
            namespace: Some(namespace.clone()),
            ..Default::default()
        },
        rules: Some(vec![
    ...
    let operator_role_binding = RoleBinding {
        metadata: ObjectMeta {
            name: Some(format!("{}-rolebinding", operator_service_account_name)),
            namespace: Some(namespace.clone()),
            ..Default::default()
        },
        role_ref: RoleRef {
            ...
            kind: "Role".to_string(),
            name: format!("{}-role", operator_service_account_name),
        },
    ...
    let compute_pcrs_role = Role {
        metadata: ObjectMeta {
            name: Some(format!("{}-role", compute_pcrs_service_account_name)),
    ...
    let compute_pcrs_role_binding = RoleBinding {
        metadata: ObjectMeta {
            name: Some(format!("{}-rolebinding", compute_pcrs_service_account_name)),
            namespace: Some(args.trustee_namespace.clone()),
            ..Default::default()
        },
        role_ref: RoleRef {
            api_group: "rbac.authorization.k8s.io".to_string(),
            kind: "Role".to_string(),
            name: format!("{}-role", compute_pcrs_service_account_name),
        },
        subjects: Some(vec![Subject {
            kind: "ServiceAccount".to_string(),
